...
DIRECT_PROXY_FILE = DATA_DIR / "direct_proxy_url.txt"
...
def get_saved_direct_proxy() -> str | None:
    if DIRECT_PROXY_FILE.exists():
        val = DIRECT_PROXY_FILE.read_text(encoding="utf-8").strip()
        return val or None
    return None


def save_direct_proxy(url: str):
    DIRECT_PROXY_FILE.write_text(url.strip(), encoding="utf-8")


def clear_direct_proxy():
    DIRECT_PROXY_FILE.unlink(missing_ok=True)


def resolve_active_proxy() -> tuple[str | None, str | None, str | None]:
    """اولویت‌بندی منبع پروکسی. خروجی: (mode, value, source)
    mode یکی از 'direct' (پروکسی HTTP/SOCKS5 معمولی) یا 'vless' یا None."""
    env_direct = os.getenv("IG_PROXY", "").strip()
    if env_direct:
        return "direct", env_direct, "متغیر IG_PROXY در Railway"

    env_vless = os.getenv("PROXY_VLESS_URL", "").strip()
    if env_vless:
        return "vless", env_vless, "متغیر PROXY_VLESS_URL در Railway"

    saved_direct = get_saved_direct_proxy()
    if saved_direct:
        return "direct", saved_direct, "پنل مدیریت"

    saved_vless = get_saved_vless_url()
    if saved_vless:
        return "vless", saved_vless, "پنل مدیریت"

    return None, None, None


def set_direct_proxy(url: str) -> tuple[bool, str]:
    """یک پروکسی HTTP/SOCKS5 معمولی (نه VLESS) رو مستقیم فعال می‌کنه."""
    global IG_PROXY
    stop_xray()
    clear_vless_url()
    save_direct_proxy(url)
    IG_PROXY = url.strip()
    return True, IG_PROXY


def remove_active_proxy():
    """هر دو نوع پروکسی (مستقیم و VLESS) رو کامل غیرفعال می‌کنه."""
    global IG_PROXY
    stop_xray()
    clear_vless_url()
    clear_direct_proxy()
...
def proxy_status_text() -> str:
    if not IG_PROXY:
        return "غیرفعال ❌"
    mode, _, source = resolve_active_proxy()
    if mode == "direct":
        return f"فعال ✅ (مستقیم، منبع: {source})"
    if mode == "vless":
...
def admin_panel_keyboard(context: ContextTypes.DEFAULT_TYPE) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        [
            ...
            [InlineKeyboardButton("🔌 تنظیم پروکسی مستقیم (HTTP/SOCKS5)", callback_data="admin:set_direct_proxy")],
            [InlineKeyboardButton("🧪 تست پروکسی", callback_data="admin:test_proxy")],
            [InlineKeyboardButton("🗑 حذف پروکسی", callback_data="admin:remove_proxy")],
        ]
    )
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "stats":
        ...
        if mode == "direct":
            lines.append(f"نوع: پروکسی مستقیم HTTP/SOCKS5\nمنبع: {source}")
        elif mode == "vless":
            lines.append(f"نوع: VLESS (از طریق xray)\nمنبع: {source}")
        else:
            lines.append("هیچ پروکسی‌ای تنظیم نشده.")
        await query.edit_message_text("\n".join(lines), reply_markup=admin_panel_keyboard(context))

    elif action == "set_vless":
        if os.getenv("IG_PROXY", "").strip() or os.getenv("PROXY_VLESS_URL", "").strip():
            await query.edit_message_text(
                "⚠️ یکی از متغیرهای IG_PROXY یا PROXY_VLESS_URL در تنظیمات Railway ست شده "
                "و همیشه اولویت داره.\nبرای مدیریت پروکسی از همینجا (پنل)، اول اون متغیر رو "
                "از Variables توی Railway پاک کن، بعد دوباره امتحان کن.",
                reply_markup=admin_panel_keyboard(context),
            )
            return
        ...
        if os.getenv("IG_PROXY", "").strip() or os.getenv("PROXY_VLESS_URL", "").strip():
            ...
            return
        pending_input[chat_id] = "set_direct_proxy"
        await query.edit_message_text(
            "آدرس کامل پروکسی رو بفرست، مثال:\n"
        ...
        await asyncio.to_thread(remove_active_proxy)
        await query.edit_message_text(
            "🗑 پروکسی متوقف و حذف شد. از این به بعد اینستاگرام بدون پروکسی تلاش می‌کنه.",
            reply_markup=admin_panel_keyboard(context),
        )
...
async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if pending == "set_direct_proxy" and is_admin(user.id if user else None):
        url = text.strip()
        valid_prefixes = ("http://", "https://", "socks5://", "socks5h://", "socks4://")
        if not url.startswith(valid_prefixes):
            pending_input[chat_id] = "set_direct_proxy"
            await update.message.reply_text(
                "فرمت درست نیست؛ باید با http:// یا socks5:// (و مشابه) شروع بشه. دوباره بفرست:"
            )
            return

        ok, msg = await asyncio.to_thread(set_direct_proxy, url)
        await update.message.reply_text(
            f"✅ پروکسی مستقیم تنظیم شد.\nمقدار: {msg}\n\n"
            "پیشنهاد می‌کنم الان «🧪 تست پروکسی» رو بزنی، اگه سالم بود بعدش "
            "«🔁 ورود مجدد به اینستاگرام» رو بزن.",
            reply_markup=admin_panel_keyboard(context),
        )
...
async def post_init(application: Application):
    ...
    # اگه پروکسی مستقیم از env (IG_PROXY) نیومده، ببین از پنل مدیریت یا env
    # چیزی (مستقیم یا VLESS) ذخیره شده که باید فعالش کنیم
    if not IG_PROXY:
        mode, value, source = resolve_active_proxy()
        if mode == "direct":
            IG_PROXY = value
            log.info("پروکسی مستقیم (منبع: %s) فعال شد.", source)
        elif mode == "vless":
            ok, msg = await asyncio.to_thread(start_xray, value)
            if ok:
